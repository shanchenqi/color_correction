...
Mat saturate(Mat src, double low, double up) {
    ...
    for (int i = 0; i < src.rows; i++) {//todo 一行for循环
...
Mat lab2xyz(Mat lab, IO io) {
   ...
   // map <IO, vector<double>> xyz_ref_white = get_xyz_ref_white();
    vector<double> xyz_ref_white_io = illuminants[io];
  //  double Xn = xyz_ref_white_io[0];
  //  double Yn = xyz_ref_white_io[1];
  //  double Zn = xyz_ref_white_io[2];
...
//
         //   double l = lab.at<Vec3d>(i, j)[0];        //todo 一行
...
Mat rgb2gray(Mat rgb) {//Mat引用
    Mat togray = (Mat_<double>(3, 1) << 0.2126, 0.7152, 0.0722);//全局变量放namespace

    Mat gray(rgb.rows, rgb.cols, CV_64FC1);
    for (int i = 0; i < rgb.rows; i++) {
        for (int j = 0; j < rgb.cols; j++) {

            double res1 = rgb.at<Vec3d>(i, j)[0] * togray.at<double>(0, 0);//todo
...
Mat gamma_correction(cv::Mat& src, float K) {
    ...
    for (int row = 0; row < src.rows; row++) {
        for (int col = 0; col < src.cols; col++) {
           ...
           // dst.at<Vec3d>(row, col)[0] = gamma_correction_f(src.at<Vec3d>(row, col)[0], K);//写一个循环
