...
Mat distance_s(Mat Lab1, Mat Lab2,string distance) {
	Mat distance_lab;
	if (Lab1.channels() == 1) {
		Mat distance_lab(Lab1.rows, 1, CV_64FC1);
		
		for (int i = 0; i < Lab1.rows; i++) {
			double* datasrc1 = Lab1.ptr<double>(i);
			double l1 = datasrc1[0];
			double a1 = datasrc1[1];
			double b1 = datasrc1[2];
			double* datasrc2 = Lab2.ptr<double>(i);
			double l2 = datasrc2[0];
			double a2 = datasrc2[1];
			double b2 = datasrc2[2];
			LAB lab1 = { l1,a1,b1 };
			LAB lab2 = { l2,a2,b2 };
			
			if (distance == "distance_de94") {
				distance_lab.at<double>(i, 0) = deltacE_ciede94(lab1, lab2);
			}
			else if (distance == "distance_de76") {
				
				distance_lab.at<double>(i, 0) = deltaE_cie76(lab1, lab2);
			}
			else if (distance == "distance_cmc") {
				distance_lab.at<double>(i, 0) = deltacE_cmc(lab1, lab2);
			}
			else if (distance == "distance_de00") {
				distance_lab.at<double>(i, 0) = deltacE_ciede2000(lab1, lab2);
			}
			else {
