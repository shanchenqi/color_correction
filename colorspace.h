...
public:
	...
	//RGB_Base_(IO io, double xr, ...) :IO(io), ...{};
	//RGB_Base_(bool linear=false) :ColorSpace(),linear(linear) {
	RGB_Base_(bool linear = false) :linear(linear) {
		//this->io = io;
		////add_value();
		_cal_linear();
		_cal_M();
	...
	//void _cal_M() {
	virtual void _cal_M(void) {
		std::cout <<"io.illuminant"<< io.illuminant <<std:: endl;
		...
		//XYZ_rgbl.reshape(1, XYZ_rgbl.rows);
		Mat XYZw = Mat(illuminants[this->io], true);
		std::cout<<io.illuminant<< std::endl;
		std::cout <<"XYZ_rgbl.size() "<< XYZ_rgbl.size() <<"  XYZw.size() "<< XYZw.size() <<" Srgb.size() " <<Srgb.size() << std::endl;
		std::cout << "XYZ_rgbl.size() " << XYZ_rgbl.channels()<< "  XYZw.size() " << XYZw.channels()<< " Srgb.size() " << Srgb.channels() << std::endl;
...
public:
	using RGB_Base_::RGB_Base_;
	//sRGB_Base(bool linear,IO io,double xr):a(a), alpha(alpha),RGB_Base_(linear,io,xr){
	
	sRGB_Base_(bool linear = false) :RGB_Base_(linear) {
	}
	double a=0.055;
	double gamma=2.4;
	double alpha;
	...
	Mat _toL(Mat rgb) {
		//[x](double element)->double {return _gamma_correction(element, gamma)
...
public:
	//sRGB(bool linear):sRGB_Base_(io, )
	//sRGB() :io(D65_2), ...{}
	//io = D65_2;
	//sRGB_(bool linear, IO io, double xr) :xr(0.64), yr(0.33), xg(0.3), yg(0.6), xb(0.15), yb(0.06), a(0.055), gamma(2.4), RGB_Base_(linear, io, xr) {};
	...
	//sRGB_(bool linear = false) :sRGB_Base_(linear,D65_2,0.64,0.33), io(D65_2), linear(linear){
	sRGB_(bool linear = false):sRGB_Base_(linear ){
		io = D65_2;
		xr = 0.64;
		yr = 0.33;
		xg = 0.3;
...
public:
	...
	AdobeRGB_(bool linear = false) :AdobeRGB_Base_(linear) {
		io = D65_2;
		type = "AdobeRGB";
		xr = 0.64;
		yr = 0.33;
...
public:
	...
	Mat _to(Mat src) {
		...
		//[this](double a)->double {return _toL_ew(a); }
